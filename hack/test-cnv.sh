...
KUBEVIRT_TAG=$(docker run quay.io/skopeo/stable:latest inspect --creds "$BREW_IMAGE_REGISTRY_USERNAME:$(<$BREW_IMAGE_REGISTRY_TOKEN_PATH)" \
  docker://${VIRT_OPERATOR_IMAGE} | jq '.Labels["upstream-version"]')
